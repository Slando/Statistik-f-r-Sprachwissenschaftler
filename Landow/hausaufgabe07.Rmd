...
ggplot(data = noten.dist, aes(x = Notenpunkte, y = Anzahl, color = Standardabweichung)) + 
    geom_line() + scale_x_continuous(limits = c(-1, 17))
```
...
Die Benotung ist abweichend. Es gibt zumindest bei 50 Studenten eine signifikante Abweichung, da `r sqrt(50) * ((10 - 8)/3)` > `r qnorm(0.94)`. Bei 7 Studenten ist die Abweichung nicht signifikant.
...
print(z)
```
Der Notenschnitt weicht für 25 Studenten im Schnitt signifikant (da `r z` > `r qnorm(0.94)`) von der Erwartung (8NP) ab, nicht jedoch für 20 Studenten.
